...
/**
 * Determines the optimal depth 'k' for the fitness function based on saturation.
 * Uses the binomial coefficients to compare the current budget against total possibilities.
 *
 * @param nbTickets   Number of tickets in your budget.
 * @param alphabet    The pool of numbers (e.g., [1, 2, ..., 50]).
 * @param ticketSize  The number of balls per ticket (default 5).
 * @return            The optimal k-value (1, 2, or 3).
 */
public static getOptimalK(nbTickets: number, alphabet: Tuple, ticketSize: number): number {
    const n = alphabet.length;

    for (let k = 1; k <= 3; k++) {
        // 1. Total unique combinations of size k possible in the alphabet
        const totalPossible = Number(TupleHelper.binomial(n, k));

        // 2. Number of combinations of size k generated by your budget
        ...
        // 3. Selection Logic:
        // Use the SATURATION_THRESHOLD to decide if we need to go deeper
    ...
    //console.log(`[Optimizer] Selected k=3 (High density system)`);
    return 3;
}
